                    ...
                    <div class="row">
                        <div class="col-12 col-lg-4">
                            <div class="d-flex align-items-center">
                                ...
                                <div class="text-truncate font-weight-medium">{{ __('Google Safe Browsing') }}</div>
                            </div>
                        </div>
                            ...
                            @else
                                @foreach($report['results'][$name]['errors'] as $error => $details)
                                    <div class="{{ (!$loop->first) ? 'mt-3' : ''}}">
                                        @if($error == 'failed')
                                            {{ __('The webpage is not in good standing.') }}
                                        @endif
                                    </div>
                                @endforeach
                            @endif
            ...
            <div class="alert alert-secondary mb-0">
                ...
                <div class="row">
                    ...
                    <div class="col-12 col-md-auto">
                        <a href="https://developers.google.com/search/docs/advanced/security/malware?hl=en&ref_topic=4596795" class="alert-link font-weight-medium d-flex align-items-center" target="_blank" rel="nofollow">Google @include('icons.external', ['class' => 'fill-current width-3 height-3 ' . (__('lang_dir') == 'rtl' ? 'mr-1' : 'ml-1')])</a>
                    </div>
                </div>
            </div>
        </div>
